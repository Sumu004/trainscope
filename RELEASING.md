...
This project ships to [PyPI](https://pypi.org/project/pytscope/) via **PyPI
...
pytscope follows [Semantic Versioning](https://semver.org/spec/v2.0.0.html):
...
The version lives in **one place**: `pytscope/__init__.py` (`__version__`).
...
1. On **PyPI** → project `pytscope` → *Settings → Publishing → Add a pending
   publisher*:
   - Owner: `Sumu004`, Repository: `pytscope`
...
2. **Bump the version** in `pytscope/__init__.py`.
...
5. **(Optional) Dry-run to TestPyPI:** Actions → *Publish to PyPI* → *Run
   ...
   pip install --index-url https://test.pypi.org/simple/ \
       --extra-index-url https://pypi.org/simple/ pytscope
   ```
6. **Tag and push** from the merged commit:
   ```bash
   git tag -a vX.Y.Z -m "pytscope vX.Y.Z"
...
- Confirm the new version installs cleanly: `pip install pytscope==X.Y.Z`.
